#!/bin/bash
# Quick test run for login nodes - validates features before full cluster runs
# Usage: bash experiments/snake/test_login_node.sh
# Runs: 1 critic warmup step, 3 training steps with small batch sizes

set -e

project_name=verl_env
experiment_name=test_login_node
run_number=1
number_of_gpus=1
