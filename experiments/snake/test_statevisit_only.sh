#!/bin/bash
# Minimal test: StateVisitation eval only (20 rollouts, 4 steps)
# Verifies game_state_text dedup fix without heavy inference
# Usage: bash experiments/snake/test_statevisit_only.sh

set -e

project_name=verl_env
experiment_name=test_statevisit
number_of_gpus=1
...
PYTHONUNBUFFERED=1 python3 -m verl.trainer.main_ppo \
  ...
  evaluation=snake_evals_statevisit_only \
  prompt=snake \
  prompt.prompt.multi_action_reasoning=True \
  prompt.prompt.epsilon=0.1 2>&1 | tee test_statevisit.log

echo ""
echo "=== StateVisitation Results ==="
grep -E "n_distinct_state_actions|state_action_coverage|StateVisitation" test_statevisit.log || echo "No StateVisitation metrics found"
echo ""
echo "=== Errors ==="
grep -iE "ValueError|game_state_text|Traceback" test_statevisit.log || echo "No errors found"
echo "Done!"
