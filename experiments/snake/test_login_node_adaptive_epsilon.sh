#!/bin/bash
# Test adaptive epsilon-greedy exploration
# Usage: bash experiments/snake/test_login_node_adaptive_epsilon.sh
# Runs: 1 critic warmup step, 3 training steps with adaptive epsilon enabled
# Window=3 so adaptive kicks in quickly during the short test

set -e

project_name=verl_env
experiment_name=test_login_node_adaptive_epsilon
run_number=1
number_of_gpus=1
...
PYTHONUNBUFFERED=1 python3 -m verl.trainer.main_ppo \
  ...
  evaluation=snake_evals_minimal \
  prompt=snake \
  prompt.prompt.multi_action_reasoning=true \
  prompt.prompt.adaptive_epsilon.enabled=true \
  prompt.prompt.adaptive_epsilon.epsilon_max=0.3 \
  prompt.prompt.adaptive_epsilon.window_size=3 \
  prompt.prompt.adaptive_epsilon.k=5.0 \
  2>&1 | tee test_login_node_adaptive_epsilon.log

echo ""
echo "=== Test Summary ==="
echo "Look for the following in the log:"
echo "  1. '[Trainer] Adaptive epsilon enabled' - confirms init"
echo "  2. adaptive_epsilon/value metric = 0 during warmup (first 3 steps)"
echo "  3. adaptive_epsilon/buffer_fill increasing from 0.33 to 1.0"
echo "  4. train/episode_return_mean metric logged each step"
echo "  5. No crashes"
echo ""
