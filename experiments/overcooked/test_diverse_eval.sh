#!/bin/bash
# Test script for parallel diverse evaluation on Overcooked
# Validates the diverse prompting feature before full cluster runs
#
# Usage: bash experiments/overcooked/test_diverse_eval.sh
#
# Runs: 1 critic warmup step, 1 training step, with diverse eval at start
# Tuned for 24GB L4 GPUs

set -e

project_name=verl_env
experiment_name=test_diverse_eval
run_number=1
number_of_gpus=1
...
export VERL_MULTIENV_EVALUATOR_DEBUG=1  # Enable detailed eval logging
...
# Tuned for 24GB L4 GPUs
micro_batch_size=2
max_prompt_length=512  # Slightly larger for Overcooked prompts
...
# For diverse eval with K=5 prompts, we're doing 5x inference
# Reduce n_rollouts to compensate for memory
n_rollouts=4  # Will become 4*5=20 inference calls per step
...
echo "=========================================="
echo "Testing Parallel Diverse Evaluation"
echo "Model: $MODEL_ID"
echo "Environment: Overcooked (cramped_room)"
echo "Diverse prompts: 5 (baseline, cautious, aggressive, strategic, cooperative)"
echo "=========================================="
...
PYTHONUNBUFFERED=1 python3 -m verl.trainer.main_ppo \
  ...
  prompt=overcooked 2>&1 | tee test_diverse_eval.log

echo ""
echo "=========================================="
echo "Test complete! Check log for diverse/* metrics:"
echo "  grep 'diverse/' test_diverse_eval.log"
echo "=========================================="
