...
def test_initial_supplement_is_min():
    ad = AdaptiveDIME(supplement_min=0.1, supplement_max=0.9, window_size=5, k=5.0, inflection=0.5)
...
def test_flat_rewards_high_supplement():
    ...
    ad = AdaptiveDIME(supplement_min=0.0, supplement_max=0.875, window_size=5, k=5.0, inflection=0.5)
    for _ in range(5):
...
def test_inflection_zero_gives_mid_on_flat():
    ...
    ad = AdaptiveDIME(supplement_min=0.1, supplement_max=0.9, window_size=5, k=5.0, inflection=0.0)
    for _ in range(5):
        ad.update(1.0)
...
def test_supplement_stays_in_bounds():
    ad = AdaptiveDIME(supplement_min=0.1, supplement_max=0.9, window_size=3, k=50.0, inflection=0.5)
    # Extreme positive slope
    for v in [0.0, 100.0, 200.0]:
        ad.update(v)
    assert ad.current_supplement_prob >= 0.1
    # Extreme negative slope
    ad2 = AdaptiveDIME(supplement_min=0.1, supplement_max=0.9, window_size=3, k=50.0, inflection=0.5)
...
def test_metrics_keys():
    ad = AdaptiveDIME(supplement_min=0.1, supplement_max=0.9, window_size=3, k=5.0, inflection=0.5)
    for i in range(3):
        ad.update(float(i))
    m = ad.get_metrics()
    assert "dime/adaptive_supplement_prob" in m
    assert "dime/adaptive_slope" in m
    assert "dime/adaptive_inflection" in m
    assert "dime/adaptive_buffer_fill" in m
    assert m["dime/adaptive_buffer_fill"] == 1.0
    assert m["dime/adaptive_inflection"] == 0.5
