...
class TestJaxForkGuard:
    """Tests for the JAX + fork safety guard."""

    def test_jax_guard_raises_when_jax_imported(self):
        """VecEnv raises if JAX is imported and mp_method is fork."""
        from verl.envs.environments import make_env
        from verl.envs.captioners import make_captioner
        from verl.envs.vec_env import VecEnv
        import os
        ...
        old_env = os.environ.pop('VERL_ALLOW_UNSAFE_FORK_WITH_JAX', None)

        try:
            # Simulate JAX being imported
            # (We can't actually import JAX in this test, but we can mock sys.modules)
            fake_jax_modules = ['jax', 'jax.numpy', 'jaxlib']
            for mod in fake_jax_modules:
                if mod not in sys.modules:
                    sys.modules[mod] = type(sys)('fake_jax')

            # CRITICAL: n_rollouts must match len(env_fns) to avoid assertion failure
            # before the JAX guard is reached
            ...
            with pytest.raises(RuntimeError, match="Cannot use fork multiprocessing when JAX is already imported"):
                VecEnv(
                    env_name="fastsnake",
                    config=config,
            ...
            for mod in fake_jax_modules:
                if mod in sys.modules and hasattr(sys.modules[mod], '__name__') and sys.modules[mod].__name__ == 'fake_jax':
                    del sys.modules[mod]
            if old_env is not None:
                os.environ['VERL_ALLOW_UNSAFE_FORK_WITH_JAX'] = old_env

    def test_jax_guard_can_be_overridden(self):
        """VERL_ALLOW_UNSAFE_FORK_WITH_JAX=1 bypasses the guard."""
        from verl.envs.environments import make_env
        from verl.envs.captioners import make_captioner
        from verl.envs.vec_env import VecEnv
        import os

        old_env = os.environ.get('VERL_ALLOW_UNSAFE_FORK_WITH_JAX')

        try:
            # Set override
            os.environ['VERL_ALLOW_UNSAFE_FORK_WITH_JAX'] = '1'
            ...
            # Should NOT raise with override set
            env = VecEnv(
                env_name="fastsnake",
                config=config,
                env_fns=[get_env_fn(0)],
            ...
            if old_env is not None:
                os.environ['VERL_ALLOW_UNSAFE_FORK_WITH_JAX'] = old_env
            elif 'VERL_ALLOW_UNSAFE_FORK_WITH_JAX' in os.environ:
                del os.environ['VERL_ALLOW_UNSAFE_FORK_WITH_JAX']
